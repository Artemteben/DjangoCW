...
def get_cron_trigger(mailing):
    ...
    if mailing.frequency == Mailing.Frequency.DAY:
        ...
        logger.error(f"Частота рассылки будет минутная")
        return CronTrigger(minute= '1')
...
def schedule_future_mailing(scheduler, mailing):
    ...
    if mailing.date_time_last_mailing and mailing.date_time_last_mailing <= now:
        logger.info(f"Рассылка {mailing.id} пропущена: время завершения истекло.")
        return
    ...
    if trigger:
        ...
        logger.info(f"Запланирована рассылка {mailing.id} с частотой {mailing.frequency}.")
...
#     scheduler.add_jobstore(DjangoJobStore(), "default")
#
#     for mailing in Mailing.objects.filter(status=Mailing.Status.CREATED):
#         schedule_future_mailing(scheduler, mailing)
#
#     scheduler.add_job(
#         delete_old_job_executions,
...
def scheduler_started():
    ...
    try:
        ...
        for mailing in Mailing.objects.filter(
            ...
            logger.info(f"Запланирована рассылка ID: {mailing.id}.")
            schedule_future_mailing(scheduler, mailing)
